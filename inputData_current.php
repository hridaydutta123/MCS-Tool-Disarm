...
function insert_file_record($name, $type, $ttl, $c_id, $r_id, $latitude, $longitude, $ts, $size, $mod_date, $file_content) {
    $sql = sprintf("INSERT INTO file_details SET filename = '%s', type = '%s', ttl = '%s', c_id = '%s', r_id = '%s', latitude = '%s', longitude = '%s', ts = '%s', size = '%s', lastdate = '%s', serial_no = '%s', text = '%s'", $name, $type, $ttl, $c_id, $r_id, $latitude, $longitude, $ts, $size, $mod_date, $file_content);
    if (mysql_query($sql)) {
        return true;
    } else {
        return false;
...
CREATE TABLE IF NOT EXISTS file_details (
  ...
  ttl varchar(5) NOT NULL default '',
  c_id varchar(14) NOT NULL default '',
  r_id varchar(14) NOT NULL default '',
  ...
  text varchar(5555) NOT NULL default '',
...
if ($num_files > 0) {
    foreach ($file_array as $val) {
		...
		*/
		list($type, $ttl, $c_id, $r_id, $latitude, $longitude, $ts_p) = explode("_", $val); //extract value separated by "_" in each variable
      ...
      if ($handle) {
        ...
        if (insert_file_record($val, $type, $ttl, $c_id, $r_id, $latitude, $longitude, $ts, $size, $fdate, $file_content)) {
            $success++;
			$file_content = NULL;
