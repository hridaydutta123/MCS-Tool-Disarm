...
function initialize(elementId, stateAbbr) {
    ...
    $.ajax(
        {
            url : 'http://localhost/disarm/map/fetchMedicine.php',
...
function createMap(elementId, xml) {
    var i, xmlMarker, icon, type, lat, lng, latLng,
        mapMedicine = new google.maps.Map(
    ...
    function createMarker(latlng, icon) {
        ...
        google.maps.event.addListener(
            ...
            function () {
                ...
                infoWindow.open(mapMedicine, marker);
            }
        );
        markers.push(marker);
    }
    ...
    markerCluster = new MarkerClusterer(mapMedicine, markers);
    for (i = 0; i < latlngs.length; i++) {
        bounds.extend(latlngs[i]);
    }
    mapMedicine.fitBounds(bounds);
    return mapMedicine;
}
...
<body>
	<div id="mapMedicine" style="width: 100%; height:400px;"></div>
</body>
</html>